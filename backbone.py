...
class Transition_Block(nn.Module):
    def __init__(self, c1, c2):
        ...
        self.cv3 = Conv(c2, c2, 3, (2,1),p=(1,1))
...
class Backbone(nn.Module):
    def __init__(self, transition_channels, block_channels, n, phi, pretrained=False):
        ...
        self.stem = nn.Sequential(
            Conv(1, transition_channels, 3, 1),
            Conv(transition_channels, transition_channels * 2, 3, (2,1),p=(1,1)),
            Conv(transition_channels * 2, transition_channels * 2, 3, 1),
        )
        # 320, 320, 64 => 160, 160, 128 => 160, 160, 256
        self.dark2 = nn.Sequential(
            Conv(transition_channels * 2, transition_channels * 4, 3, (2,1),p=(1,1)),
    ...
    def forward(self, x):
        x = self.stem(x).to(device)
        x = self.dark2(x).to(device)
        
        x = self.dark3(x).to(device)  #(1,64,256,3) --> (1,64,128,3) --> (1,128,128,3)
        feat1 = x.to(device)
        
        x = self.dark4(x).to(device)  # (1,128,128,3) --> (1,128,64,3) --> (1,256,64,3)
        feat2 = x.to(device)
        
        x = self.dark5(x).to(device)  # (1,256,64,3) -->  (1,256,32,3) -->(1,256,32,3)
        feat3 = x
